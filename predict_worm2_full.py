...
# 18-500-500-500-18:  90 hours


model = Sequential()  
model.add(LSTM(in_out_neurons, hidden_neurons, return_sequences=True))  
#model.add(LSTM(hidden_neurons, hidden_neurons, return_sequences=True))  
...
import h5py
import numpy as np
with h5py.File('./data/angle_vec2_ful.hdf5', 'r') as fid:
    angle_vec2_ful = fid['/angle_vec2_ful'][:]     
   
columns = ['a', 'b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r']    
data = pd.DataFrame(angle_vec2_ful, columns =  columns)    
...
# save to csv
np.savetxt("./data/predicted2_full(18-500-500-18).csv", predicted, delimiter=",")
np.savetxt("./data/y_test2_full(18-500-500-18).csv", y_test, delimiter=",")
...
for ii in range(400):
    ...
    generated2 = np.vstack((generated2, next_ske))

    x_prev = np.copy(x_now[0,])
                
# save to csv
np.savetxt("./data/generated2_full(18-500-500-18).csv", generated2, delimiter=",")       
