...
def train_test_split(df, test_size=1-0.0593):  
    """
...
file_no = 0
root = 'Z:/DLWeights/nas207-1/experimentBackup/from pc207-7/!worm_videos/copied_from_pc207-8/Andre/04-03-11/'
...
for path, subdirs, files in os.walk(root):
    for name in files:
        ...
        if file_no ==2:
            ...
            name_generated = str(file_no)+"_generated_rev2.csv"         
            np.savetxt(os.path.join(root, name_generated), eig_generated1, delimiter=",")   
            name_y_test = str(file_no)+"_y_test_rev2.csv"
