...
import h5py
import numpy as np
with h5py.File('./data/angle_vec_full10.hdf5', 'r') as fid:
    angle_vec = fid['/angle_vec'][:]     
   
columns = ['a', 'b','c','d','e','f','g','h','i','j','k','l','m','n','o','p']    
data = pd.DataFrame(angle_vec, columns =  columns)    
...
model.fit(X_train, y_train, batch_size=450, nb_epoch=200, validation_split=0.05)  
...
# save to csv
np.savetxt("./data/ang_MulLayer_predicted_full1.csv", predicted, delimiter=",")
np.savetxt("./data/ang_MulLayer_y_test_full1.csv", y_test, delimiter=",")
...
for ii in range(400):
    ...
    ''' artificially change the 1st element of predicted skeleton '''
    next_ske[0] = angle_vec[data.shape[0]-X_test.shape[0]+ii,0]
...
# save to csv
np.savetxt("./data/ang_MulLayer_generated_full1.csv", ang_generated1, delimiter=",")                