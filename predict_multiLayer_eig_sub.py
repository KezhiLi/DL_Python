...
tot_cut_no = sys.argv[2]
selet_cut_no = sys.argv[3]

with h5py.File(file_name+'_eig.hdf5', 'r') as fid:
    eig_coef_all = fid['/eig_coef'][:]     
    eig_coef = eig_coef_all[][]
...
# save to csv
np.savetxt(file_name+"_predicted-hid2-3.csv", predicted, delimiter=",")
np.savetxt(file_name+"_y_test-hid2-3.csv", y_test, delimiter=",")
...
# save to csv
np.savetxt(file_name+"_generated-hid2-3.csv", eig_generated1, delimiter=",")     

print time.time() - t0         
# save model weights
model.save_weights(file_name+"_generated-hid2-3.h5")      