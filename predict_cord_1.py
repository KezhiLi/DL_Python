"""
Created on Thu Jun 30 15:49:53 2016
@author: kezhili
"""
...
import h5py
import numpy as np
with h5py.File('./data/cord_1.hdf5', 'r') as fid:
    cord_1 = fid['/cord_1'][:]     
   
#columns = ['a', 'b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r']   
columnss = np.array(range(0,in_out_neurons))  
data = pd.DataFrame(cord_1, columns =  columnss)    
...
# save to csv
np.savetxt("./data/predicted_cord_1.csv", predicted, delimiter=",")
np.savetxt("./data/y_test_cord_1.csv", y_test, delimiter=",")
...
for ii in range(400):
    ...
    generated = np.vstack((generated, next_ske))

    x_prev = np.copy(x_now[0,])
                
# save to csv
np.savetxt("./data/generated_cord_1.csv", generated, delimiter=",")                