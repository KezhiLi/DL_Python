...
model = Sequential()  
model.add(LSTM(5, 500, return_sequences=True))  
model.add(LSTM(500, 500, return_sequences=False))  
model.add(Dropout(0.2))  
model.add(Dense(500, 5))  
model.add(Activation("linear"))  
model.compile(loss="mean_squared_error", optimizer="rmsprop")  
...
import h5py
import numpy as np
with h5py.File('./data/eig_para_full1.hdf5', 'r') as fid:
...
# save to csv
np.savetxt("./data/eig_MulLayer_predicted_full1.csv", predicted, delimiter=",")
np.savetxt("./data/eig_MulLayer_y_test_full1.csv", y_test, delimiter=",")
...
# save to csv
np.savetxt("./data/eig_MulLayer_generated_full1.csv", eig_generated1, delimiter=",")     
